...
class CommonAgent(a2c_continuous.A2CAgent):
    ...
    def _assemble_train_info(self, train_info, frame):
        ...
        if "prior_disc_loss" in train_info:
            train_info_dict.update(
                {
                    "prior_disc_loss": torch_ext.mean_list(train_info['prior_disc_loss']).item(),
                    "prior_logits": torch_ext.mean_list(train_info['prior_logits']).item(),
                }
            )
...
class CommonDiscreteAgent(a2c_discrete.DiscreteA2CAgent):
    ...
    def _assemble_train_info(self, train_info, frame):
        train_info_dict = {
            ...
            "clip_frac": torch_ext.mean_list(train_info['actor_clip_frac']).item(),
            "kl": torch_ext.mean_list(train_info['kl']).item(),
            # "prior_disc_loss": torch_ext.mean_list(train_info['prior_disc_loss']).item() if 'prior_disc_loss' in train_info else 0.0,
        }
